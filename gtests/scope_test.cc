...
TEST_F(ScopeTest, index_operator) {
    std::string text{ "Łódź Pszczółkowski"};
    ppx::utf8::span span{text};
...
TEST_F(ScopeTest, tolower_test) {
    ...
    for (auto const& tt : tests) {
        ...
        ASSERT_EQ(std::string(scope.to_lower().data()), tt.expected);
    }
}
...
TEST_F(ScopeTest, toupper) {
    ...
    for (auto const& tt : tests) {
        auto scope = ppx::utf8::span{tt.text}();
        ASSERT_EQ(std::string(scope.to_upper().data()), tt.expected);
    }
}
